...
- **分支管理** — 可选地为每个规格创建和切换 `spex/<topic>` 分支，
  支持自动合并或创建 PR。
- **支持 worktree 和子模组** — 规格锚定到主 worktree，所有 worktree
  共享同一个规格存储；同时支持子模组。
- **模糊主题匹配** — 所有命令接受部分主题名称，支持交互式消歧。
...
| 命令             | 说明                        |
|------------------|----------------------------|
| `spex list`      | 列出规格主题及状态和进度       |
| `spex show`      | 显示主题概要信息              |
| `spex open`      | 在系统文件浏览器中打开主题目录  |
...
列出所有规格主题，显示状态图标和进度比率：

- `spex list` — 紧凑视图：主题名称、状态和进度（如 `3/5`）。
- `spex list -v` — 增加主题描述。
- `spex list -vv` — 增加各步骤的完成状态列表。
- `spex list --all` — 包含所有仓库的主题，不限于当前仓库。

#### `spex show <topic>`

- `spex show <topic>` — 显示主题概要（状态、日期、分支、进度）。
- `spex show -v <topic>` — 显示完整的规格内容和结构化的待办详情。
